  ...
  methods: {
    ...
    async signUp() {
      if (this.signup.password != this.signup.passwordCheck) {
        ...
        await axios.post(`${process.env.VUE_APP_SERVER_URL}/api/auth/signup`, {
    ...
    async signIn() {
      if (
        ...
        await axios.post(`${process.env.VUE_APP_SERVER_URL}/api/auth/signin`, {
    ...
    async emailAuth() {
      ...
      await axios.get(`${process.env.VUE_APP_SERVER_URL}/api/member/emailAuth?email=` + this.signup.email) // 이메일 전송하기
        .then((res) => {
