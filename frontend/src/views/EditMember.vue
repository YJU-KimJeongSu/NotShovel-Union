  ...
  methods: {
    async editUserInfo() {
      ...
      if (this.checkingText === '비어있는 칸이 있습니다') {
        ...
        axios.patch("http://process.env.SERVER_URL/api/member/edit", {
    ...
    async deleteUser() {
      if (this.checkingText === '비어있는 칸이 있습니다') {
        ...
        await axios.delete('http://process.env.SERVER_URL/api/member/delete', {
    ...
    //     const { data: { filename } } = await axios.post('http://process.env.SERVER_URL/api/member/image', formData, header);
    //     return filename;
    //   }
    // }
  },
