...
export default {
  ...
  async mounted() {
    ...
    // 게시판 리스트 겟요청
    await axios.get(`${process.env.VUE_APP_SERVER_URL}/api/board`, {
      params: {
        project_id: this.project_id
      }, 
      headers: this.$store.getters.headers
  ...
  methods: {
    ...
    async createBoardItem(boardInfo) {
      ...
      await axios.post(`${process.env.VUE_APP_SERVER_URL}/api/board`, boardInfo,{
        params: {
          project_id: this.project_id
        }, 
        headers: this.$store.getters.headers
    ...
    async saveOrderItems(newOrder) {
      ...
      await axios.patch(`${process.env.VUE_APP_SERVER_URL}/api/board/order`, bList, {
        params: {
          project_id: this.project_id
        }, headers: this.$store.getters.headers
      })
