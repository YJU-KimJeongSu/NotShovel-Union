...
export default {
  ...
  methods: {
    async goEditor() {  
      try {
        ...
        await axios.post(`${process.env.VUE_APP_SERVER_URL}/api/meeting`, {
    ...
    async getDetailMeetingMinute(_id) {
      try {
        ...
        const res = await axios.get(`${process.env.VUE_APP_SERVER_URL}/api/meeting/${boardId}/${_id}`,{
          headers: this.$store.getters.headers
        });
  ...
  async mounted() {
    ...
    try {
      const boardId = sessionStorage.getItem('board_id');
      const res = await axios.get(`${process.env.VUE_APP_SERVER_URL}/api/meeting/` + boardId, {
        headers: this.$store.getters.headers
      });
